...
APP_NAME="tectonic"
APP_VERSION="0.17.0"
if [ -n "${TECTONIC_DOWNLOAD_URL:-}" ]; then
    ARTIFACT_DOWNLOAD_URLS="$TECTONIC_DOWNLOAD_URL"
elif [ -n "${INSTALLER_DOWNLOAD_URL:-}" ]; then
    ARTIFACT_DOWNLOAD_URLS="$INSTALLER_DOWNLOAD_URL"
elif [ -n "${TECTONIC_INSTALLER_GHE_BASE_URL:-}" ]; then
    INSTALLER_BASE_URL="$TECTONIC_INSTALLER_GHE_BASE_URL"
    ARTIFACT_DOWNLOAD_URLS="${INSTALLER_BASE_URL}/tectonic-typesetting/tectonic/releases/download/tectonic@0.17.0"
elif [ -n "${TECTONIC_INSTALLER_GITHUB_BASE_URL:-}" ]; then
    INSTALLER_BASE_URL="$TECTONIC_INSTALLER_GITHUB_BASE_URL"
    ARTIFACT_DOWNLOAD_URLS="${INSTALLER_BASE_URL}/tectonic-typesetting/tectonic/releases/download/tectonic@0.17.0"
else
    ARTIFACT_DOWNLOAD_URLS="https://github.com/tectonic-typesetting/tectonic/releases/download/tectonic@0.17.0"
fi
if [ -n "${TECTONIC_PRINT_VERBOSE:-}" ]; then
    PRINT_VERBOSE="$TECTONIC_PRINT_VERBOSE"
else
    PRINT_VERBOSE=${INSTALLER_PRINT_VERBOSE:-0}
fi
if [ -n "${TECTONIC_PRINT_QUIET:-}" ]; then
    PRINT_QUIET="$TECTONIC_PRINT_QUIET"
else
    PRINT_QUIET=${INSTALLER_PRINT_QUIET:-0}
fi
if [ -n "${TECTONIC_NO_MODIFY_PATH:-}" ]; then
    NO_MODIFY_PATH="$TECTONIC_NO_MODIFY_PATH"
else
    NO_MODIFY_PATH=${INSTALLER_NO_MODIFY_PATH:-0}
fi
if [ "${TECTONIC_DISABLE_UPDATE:-0}" = "1" ]; then
    INSTALL_UPDATER=0
else
    INSTALL_UPDATER=1
fi
UNMANAGED_INSTALL="${TECTONIC_UNMANAGED_INSTALL:-}"
...
if is_windows_posix && [ -n "${LOCALAPPDATA:-}" ]; then
    RECEIPT_HOME="$LOCALAPPDATA/tectonic"
else
    RECEIPT_HOME="${XDG_CONFIG_HOME:-$INFERRED_HOME/.config}/tectonic"
fi

usage() {
    # print help (this cat/EOF stuff is a "heredoc" string)
    cat <<EOF
tectonic-installer.sh

The installer for tectonic 0.17.0

This script detects what platform you're on and fetches an appropriate archive from
https://github.com/tectonic-typesetting/tectonic/releases/download/tectonic@0.17.0
    ...
    for arg in "$@"; do
        case "$arg" in
            ...
            --verbose)
                ...
                ;;
            --no-modify-path)
                say "--no-modify-path has been deprecated; please set TECTONIC_NO_MODIFY_PATH=1 in the environment"
                NO_MODIFY_PATH=1
                ;;
            *)
                OPTIND=1
...
json_binary_aliases() {
    ...
    "aarch64-apple-darwin")
        echo '{"tectonic":["nextonic"]}'
        ;;
    "aarch64-pc-windows-gnu")
        echo '{"tectonic.exe":["nextonic.exe"]}'
        ;;
...
select_archive_for_arch() {
    ...
    case "$_true_arch" in 
        "aarch64-apple-darwin")
            _archive="tectonic-0.17.0-aarch64-apple-darwin.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            _archive="tectonic-0.17.0-x86_64-apple-darwin.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            ;;
        "aarch64-pc-windows-gnu")
            _archive="tectonic-0.17.0-x86_64-pc-windows-gnu.zip"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "aarch64-pc-windows-msvc")
            _archive="tectonic-0.17.0-x86_64-pc-windows-msvc.zip"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            ;;
        "aarch64-unknown-linux-gnu")
            _archive="tectonic-0.17.0-aarch64-unknown-linux-musl.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "aarch64-unknown-linux-musl-static")
            ...
            ;;
        "arm-unknown-linux-gnueabihf")
            _archive="tectonic-0.17.0-arm-unknown-linux-musleabihf.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "arm-unknown-linux-musl-staticeabihf")
            ...
            ;;
        "i686-unknown-linux-gnu")
            _archive="tectonic-0.17.0-i686-unknown-linux-gnu.tar.gz"
        ...
        "x86_64-pc-windows-msvc")
            ...
            ;;
        "x86_64-unknown-linux-gnu")
            _archive="tectonic-0.17.0-x86_64-unknown-linux-gnu.tar.gz"
            if ! check_glibc "2" "17"; then
                _archive=""
            fi
            if [ -n "$_archive" ]; then
            ...
            _archive="tectonic-0.17.0-x86_64-unknown-linux-musl.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
...
install() {
    ...
    if [ -n "${TECTONIC_INSTALL_DIR:-}" ]; then
        _force_install_dir="$TECTONIC_INSTALL_DIR"
    ...
    say "Tectonic (and the Nextonic V2 CLI) installed successfully!"
