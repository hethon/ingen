...
$app_name = 'tectonic'
$app_version = '0.17.0'
if ($env:TECTONIC_DOWNLOAD_URL) {
  $ArtifactDownloadUrls = @($env:TECTONIC_DOWNLOAD_URL)
} elseif ($env:INSTALLER_DOWNLOAD_URL) {
  $ArtifactDownloadUrls = @($env:INSTALLER_DOWNLOAD_URL)
} elseif ($env:TECTONIC_INSTALLER_GHE_BASE_URL) {
  $installer_base_url = $env:TECTONIC_INSTALLER_GHE_BASE_URL
  $ArtifactDownloadUrls = @("$installer_base_url/tectonic-typesetting/tectonic/releases/download/tectonic@0.17.0")
} elseif ($env:TECTONIC_INSTALLER_GITHUB_BASE_URL) {
  $installer_base_url = $env:TECTONIC_INSTALLER_GITHUB_BASE_URL
  $ArtifactDownloadUrls = @("$installer_base_url/tectonic-typesetting/tectonic/releases/download/tectonic@0.17.0")
} else {
  $ArtifactDownloadUrls = @("https://github.com/tectonic-typesetting/tectonic/releases/download/tectonic@0.17.0")
}
...
if ($env:XDG_CONFIG_HOME) {
  ...
  $receipt_home = "${env:LOCALAPPDATA}\tectonic"
}

if ($env:TECTONIC_DISABLE_UPDATE) {
...
if ($NoModifyPath) {
    Write-Information "-NoModifyPath has been deprecated; please set TECTONIC_NO_MODIFY_PATH=1 in the environment"
}

if ($env:TECTONIC_NO_MODIFY_PATH) {
    $NoModifyPath = $true
}

$unmanaged_install = $env:TECTONIC_UNMANAGED_INSTALL
...
function Install-Binary($install_args) {
  ...
  $platforms = @{
    "aarch64-pc-windows-gnu" = @{
      "artifact_name" = "tectonic-0.17.0-x86_64-pc-windows-gnu.zip"
      "bins" = @("tectonic.exe")
      "libs" = @()
      "staticlibs" = @()
      "zip_ext" = ".zip"
      "aliases" = @{
        "tectonic.exe" = "nextonic.exe"
      }
      "aliases_json" = '{"tectonic.exe":["nextonic.exe"]}'
    }
    "aarch64-pc-windows-msvc" = @{
      "artifact_name" = "tectonic-0.17.0-x86_64-pc-windows-msvc.zip"
      "bins" = @("tectonic.exe")
      "libs" = @()
      "staticlibs" = @()
      "zip_ext" = ".zip"
...
function Download($download_url, $platforms, $arch) {
  ...
  if (($null -ne $info["updater"]) -and $install_updater) {
    ...
    $out_name = "$tmp\tectonic-update.exe"
...
function Invoke-Installer($artifacts, $platforms) {
  ...
  if (($env:TECTONIC_INSTALL_DIR)) {
    $force_install_dir = $env:TECTONIC_INSTALL_DIR
    $install_layout = "flat"
  ...
  if ($install_updater) {
    ...
    [IO.File]::WriteAllLines("$receipt_home/tectonic-receipt.json", "$receipt", $Utf8NoBomEncoding)
  }
  ...
  Write-Information "Tectonic (and the Nextonic V2 CLI) installed successfully!"
  if (-not $NoModifyPath) {
