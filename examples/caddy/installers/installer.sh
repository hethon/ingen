...
if [ -n "${CADDY_DOWNLOAD_URL:-}" ]; then
    ARTIFACT_DOWNLOAD_URLS="$CADDY_DOWNLOAD_URL"
elif [ -n "${INSTALLER_DOWNLOAD_URL:-}" ]; then
    ARTIFACT_DOWNLOAD_URLS="$INSTALLER_DOWNLOAD_URL"
elif [ -n "${CADDY_INSTALLER_GHE_BASE_URL:-}" ]; then
    INSTALLER_BASE_URL="$CADDY_INSTALLER_GHE_BASE_URL"
    ARTIFACT_DOWNLOAD_URLS="${INSTALLER_BASE_URL}/caddyserver/caddy/releases/download/v2.11.4"
elif [ -n "${CADDY_INSTALLER_GITHUB_BASE_URL:-}" ]; then
    INSTALLER_BASE_URL="$CADDY_INSTALLER_GITHUB_BASE_URL"
    ARTIFACT_DOWNLOAD_URLS="${INSTALLER_BASE_URL}/caddyserver/caddy/releases/download/v2.11.4"
else
    ARTIFACT_DOWNLOAD_URLS="https://github.com/caddyserver/caddy/releases/download/v2.11.4"
fi
if [ -n "${CADDY_PRINT_VERBOSE:-}" ]; then
    PRINT_VERBOSE="$CADDY_PRINT_VERBOSE"
else
    PRINT_VERBOSE=${INSTALLER_PRINT_VERBOSE:-0}
fi
if [ -n "${CADDY_PRINT_QUIET:-}" ]; then
    PRINT_QUIET="$CADDY_PRINT_QUIET"
else
    PRINT_QUIET=${INSTALLER_PRINT_QUIET:-0}
fi
if [ -n "${CADDY_NO_MODIFY_PATH:-}" ]; then
    NO_MODIFY_PATH="$CADDY_NO_MODIFY_PATH"
else
    NO_MODIFY_PATH=${INSTALLER_NO_MODIFY_PATH:-0}
fi
if [ "${CADDY_DISABLE_UPDATE:-0}" = "1" ]; then
    INSTALL_UPDATER=0
else
    INSTALL_UPDATER=1
fi
UNMANAGED_INSTALL="${CADDY_UNMANAGED_INSTALL:-}"
...
if is_windows_posix && [ -n "${LOCALAPPDATA:-}" ]; then
    RECEIPT_HOME="$LOCALAPPDATA/caddy"
else
    RECEIPT_HOME="${XDG_CONFIG_HOME:-$INFERRED_HOME/.config}/caddy"
fi
...
The installer for caddy 2.11.4
    ...
    for arg in "$@"; do
        case "$arg" in
            ...
            --verbose)
                ...
                ;;
            --no-modify-path)
                say "--no-modify-path has been deprecated; please set CADDY_NO_MODIFY_PATH=1 in the environment"
                NO_MODIFY_PATH=1
                ;;
            *)
                OPTIND=1
...
json_binary_aliases() {
    ...
    "powerpc64le-unknown-linux-gnu")
        echo '{}'
        ;;
    "powerpc64le-unknown-linux-musl-dynamic")
        echo '{}'
        ;;
    ...
    "s390x-unknown-linux-gnu")
        echo '{}'
        ;;
    "s390x-unknown-linux-musl-dynamic")
        echo '{}'
        ;;
...
select_archive_for_arch() {
    ...
    case "$_true_arch" in 
        "aarch64-apple-darwin")
            _archive="caddy_2.11.4_mac_arm64.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            _archive="caddy_2.11.4_mac_amd64.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            ;;
        "aarch64-pc-windows-gnu")
            _archive="caddy_2.11.4_windows_arm64.zip"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "aarch64-pc-windows-msvc")
            ...
            _archive="caddy_2.11.4_windows_amd64.zip"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            ;;
        "aarch64-unknown-freebsd")
            _archive="caddy_2.11.4_freebsd_arm64.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            ;;
        "aarch64-unknown-linux-gnu")
            _archive="caddy_2.11.4_linux_arm64.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "aarch64-unknown-linux-musl-static")
            ...
            ;;
        "arm-unknown-freebsd")
            _archive="caddy_2.11.4_freebsd_armv6.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "arm-unknown-linux-musl-staticeabihf")
            ...
            ;;
        "armv5te-unknown-linux-musleabi")
            _archive="caddy_2.11.4_linux_armv5.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            ;;
        "armv7-unknown-freebsd")
            _archive="caddy_2.11.4_freebsd_armv7.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            ;;
        "armv7-unknown-linux-gnueabihf")
            _archive="caddy_2.11.4_linux_armv7.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "armv7-unknown-linux-musl-staticeabihf")
            ...
            ;;
        "powerpc64le-unknown-linux-gnu")
            _archive="caddy_2.11.4_linux_ppc64le.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "powerpc64le-unknown-linux-musl-static")
            ...
            ;;
        "riscv64gc-unknown-linux-gnu")
            _archive="caddy_2.11.4_linux_riscv64.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "riscv64gc-unknown-linux-musl-static")
            ...
            ;;
        "s390x-unknown-linux-gnu")
            _archive="caddy_2.11.4_linux_s390x.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "x86_64-pc-windows-msvc")
            ...
            ;;
        "x86_64-unknown-freebsd")
            _archive="caddy_2.11.4_freebsd_amd64.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            ;;
        "x86_64-unknown-linux-gnu")
            _archive="caddy_2.11.4_linux_amd64.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
...
install() {
    ...
    if [ -n "${CADDY_INSTALL_DIR:-}" ]; then
        _force_install_dir="$CADDY_INSTALL_DIR"
