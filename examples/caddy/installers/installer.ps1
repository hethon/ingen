...
$app_name = 'caddy'
$app_version = '2.11.4'
if ($env:CADDY_DOWNLOAD_URL) {
  $ArtifactDownloadUrls = @($env:CADDY_DOWNLOAD_URL)
} elseif ($env:INSTALLER_DOWNLOAD_URL) {
  $ArtifactDownloadUrls = @($env:INSTALLER_DOWNLOAD_URL)
} elseif ($env:CADDY_INSTALLER_GHE_BASE_URL) {
  $installer_base_url = $env:CADDY_INSTALLER_GHE_BASE_URL
  $ArtifactDownloadUrls = @("$installer_base_url/caddyserver/caddy/releases/download/v2.11.4")
} elseif ($env:CADDY_INSTALLER_GITHUB_BASE_URL) {
  $installer_base_url = $env:CADDY_INSTALLER_GITHUB_BASE_URL
  $ArtifactDownloadUrls = @("$installer_base_url/caddyserver/caddy/releases/download/v2.11.4")
} else {
  $ArtifactDownloadUrls = @("https://github.com/caddyserver/caddy/releases/download/v2.11.4")
}
...
if ($env:XDG_CONFIG_HOME) {
  ...
  $receipt_home = "${env:LOCALAPPDATA}\caddy"
}

if ($env:CADDY_DISABLE_UPDATE) {
...
if ($NoModifyPath) {
    Write-Information "-NoModifyPath has been deprecated; please set CADDY_NO_MODIFY_PATH=1 in the environment"
}

if ($env:CADDY_NO_MODIFY_PATH) {
    $NoModifyPath = $true
}

$unmanaged_install = $env:CADDY_UNMANAGED_INSTALL
...
function Install-Binary($install_args) {
  ...
  $platforms = @{
    "aarch64-pc-windows-gnu" = @{
      "artifact_name" = "caddy_2.11.4_windows_arm64.zip"
      "bins" = @("caddy.exe")
      "libs" = @()
      "staticlibs" = @()
      "zip_ext" = ".zip"
    ...
    "x86_64-pc-windows-gnu" = @{
      "artifact_name" = "caddy_2.11.4_windows_amd64.zip"
      "bins" = @("caddy.exe")
      "libs" = @()
      "staticlibs" = @()
      "zip_ext" = ".zip"
...
function Invoke-Installer($artifacts, $platforms) {
  ...
  if (($env:CADDY_INSTALL_DIR)) {
    $force_install_dir = $env:CADDY_INSTALL_DIR
    $install_layout = "flat"
  ...
  if ($install_updater) {
    ...
    [IO.File]::WriteAllLines("$receipt_home/caddy-receipt.json", "$receipt", $Utf8NoBomEncoding)
  }
