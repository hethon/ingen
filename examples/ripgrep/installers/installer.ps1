...
$app_name = 'ripgrep'
$app_version = '15.2.0'
if ($env:RIPGREP_DOWNLOAD_URL) {
  $ArtifactDownloadUrls = @($env:RIPGREP_DOWNLOAD_URL)
} elseif ($env:INSTALLER_DOWNLOAD_URL) {
  $ArtifactDownloadUrls = @($env:INSTALLER_DOWNLOAD_URL)
} elseif ($env:RIPGREP_INSTALLER_GHE_BASE_URL) {
  $installer_base_url = $env:RIPGREP_INSTALLER_GHE_BASE_URL
  $ArtifactDownloadUrls = @("$installer_base_url/BurntSushi/ripgrep/releases/download/15.2.0")
} elseif ($env:RIPGREP_INSTALLER_GITHUB_BASE_URL) {
  $installer_base_url = $env:RIPGREP_INSTALLER_GITHUB_BASE_URL
  $ArtifactDownloadUrls = @("$installer_base_url/BurntSushi/ripgrep/releases/download/15.2.0")
} else {
  $ArtifactDownloadUrls = @("https://github.com/BurntSushi/ripgrep/releases/download/15.2.0")
}

$auth_token = $env:RIPGREP_GITHUB_TOKEN
...
if ($env:XDG_CONFIG_HOME) {
  $receipt_home = "${env:XDG_CONFIG_HOME}\ripgrep"
} else {
  $receipt_home = "${env:LOCALAPPDATA}\ripgrep"
}

if ($env:RIPGREP_DISABLE_UPDATE) {
...
if ($NoModifyPath) {
    Write-Information "-NoModifyPath has been deprecated; please set RIPGREP_NO_MODIFY_PATH=1 in the environment"
}

if ($env:RIPGREP_NO_MODIFY_PATH) {
    $NoModifyPath = $true
}

$unmanaged_install = $env:RIPGREP_UNMANAGED_INSTALL
...
function Install-Binary($install_args) {
  ...
  $platforms = @{
    "aarch64-pc-windows-gnu" = @{
      "artifact_name" = "ripgrep-15.2.0-aarch64-pc-windows-msvc.zip"
      "bins" = @("rg.exe")
      "libs" = @()
      "staticlibs" = @()
      "zip_ext" = ".zip"
    ...
    "i686-pc-windows-gnu" = @{
      "artifact_name" = "ripgrep-15.2.0-i686-pc-windows-msvc.zip"
      "bins" = @("rg.exe")
      "libs" = @()
      "staticlibs" = @()
      "zip_ext" = ".zip"
    ...
    "x86_64-pc-windows-gnu" = @{
      "artifact_name" = "ripgrep-15.2.0-x86_64-pc-windows-gnu.zip"
      "bins" = @("rg.exe")
      "libs" = @()
      "staticlibs" = @()
      "zip_ext" = ".zip"
    ...
    "x86_64-pc-windows-msvc" = @{
      "artifact_name" = "ripgrep-15.2.0-x86_64-pc-windows-msvc.zip"
      "bins" = @("rg.exe")
      "libs" = @()
      "staticlibs" = @()
      "zip_ext" = ".zip"
...
function Download($download_url, $platforms, $arch) {
  ...
  if (($null -ne $info["updater"]) -and $install_updater) {
    ...
    $out_name = "$tmp\ripgrep-update.exe"
...
function Invoke-Installer($artifacts, $platforms) {
  ...
  if (($env:RIPGREP_INSTALL_DIR)) {
    $force_install_dir = $env:RIPGREP_INSTALL_DIR
    $install_layout = "flat"
  ...
  if ($install_updater) {
    ...
    [IO.File]::WriteAllLines("$receipt_home/ripgrep-receipt.json", "$receipt", $Utf8NoBomEncoding)
  }
