...
if [ -n "${RIPGREP_DOWNLOAD_URL:-}" ]; then
    ARTIFACT_DOWNLOAD_URLS="$RIPGREP_DOWNLOAD_URL"
elif [ -n "${INSTALLER_DOWNLOAD_URL:-}" ]; then
    ARTIFACT_DOWNLOAD_URLS="$INSTALLER_DOWNLOAD_URL"
elif [ -n "${RIPGREP_INSTALLER_GHE_BASE_URL:-}" ]; then
    INSTALLER_BASE_URL="$RIPGREP_INSTALLER_GHE_BASE_URL"
    ARTIFACT_DOWNLOAD_URLS="${INSTALLER_BASE_URL}/BurntSushi/ripgrep/releases/download/15.2.0"
elif [ -n "${RIPGREP_INSTALLER_GITHUB_BASE_URL:-}" ]; then
    INSTALLER_BASE_URL="$RIPGREP_INSTALLER_GITHUB_BASE_URL"
    ARTIFACT_DOWNLOAD_URLS="${INSTALLER_BASE_URL}/BurntSushi/ripgrep/releases/download/15.2.0"
else
    ARTIFACT_DOWNLOAD_URLS="https://github.com/BurntSushi/ripgrep/releases/download/15.2.0"
fi
if [ -n "${RIPGREP_PRINT_VERBOSE:-}" ]; then
    PRINT_VERBOSE="$RIPGREP_PRINT_VERBOSE"
else
    PRINT_VERBOSE=${INSTALLER_PRINT_VERBOSE:-0}
fi
if [ -n "${RIPGREP_PRINT_QUIET:-}" ]; then
    PRINT_QUIET="$RIPGREP_PRINT_QUIET"
else
    PRINT_QUIET=${INSTALLER_PRINT_QUIET:-0}
fi
if [ -n "${RIPGREP_NO_MODIFY_PATH:-}" ]; then
    NO_MODIFY_PATH="$RIPGREP_NO_MODIFY_PATH"
else
    NO_MODIFY_PATH=${INSTALLER_NO_MODIFY_PATH:-0}
fi
if [ "${RIPGREP_DISABLE_UPDATE:-0}" = "1" ]; then
    INSTALL_UPDATER=0
else
    INSTALL_UPDATER=1
fi
UNMANAGED_INSTALL="${RIPGREP_UNMANAGED_INSTALL:-}"
...
{"binaries":["CARGO_DIST_BINS"],"binary_aliases":{},"cdylibs":["CARGO_DIST_DYLIBS"],"cstaticlibs":["CARGO_DIST_STATICLIBS"],"install_layout":"unspecified","install_prefix":"AXO_INSTALL_PREFIX","modify_path":true,"provider":{"source":"cargo-dist","version":"0.32.0"},"source":{"app_name":"ripgrep","name":"ripgrep","owner":"BurntSushi","release_type":"github"},"version":"15.2.0"}
EORECEIPT
...
if is_windows_posix && [ -n "${LOCALAPPDATA:-}" ]; then
    RECEIPT_HOME="$LOCALAPPDATA/ripgrep"
else
    RECEIPT_HOME="${XDG_CONFIG_HOME:-$INFERRED_HOME/.config}/ripgrep"
fi

usage() {
    # print help (this cat/EOF stuff is a "heredoc" string)
    cat <<EOF
ripgrep-installer.sh

The installer for ripgrep 15.2.0
    ...
    for arg in "$@"; do
        case "$arg" in
            ...
            --verbose)
                ...
                ;;
            --no-modify-path)
                say "--no-modify-path has been deprecated; please set RIPGREP_NO_MODIFY_PATH=1 in the environment"
                NO_MODIFY_PATH=1
                ;;
            *)
                OPTIND=1
...
select_archive_for_arch() {
    ...
    case "$_true_arch" in 
        "aarch64-apple-darwin")
            _archive="ripgrep-15.2.0-aarch64-apple-darwin.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            _archive="ripgrep-15.2.0-x86_64-apple-darwin.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            ;;
        "aarch64-pc-windows-gnu")
            _archive="ripgrep-15.2.0-aarch64-pc-windows-msvc.zip"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            _archive="ripgrep-15.2.0-x86_64-pc-windows-gnu.zip"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "aarch64-pc-windows-msvc")
            ...
            _archive="ripgrep-15.2.0-x86_64-pc-windows-msvc.zip"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            _archive="ripgrep-15.2.0-i686-pc-windows-msvc.zip"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "aarch64-unknown-linux-gnu")
            ...
            _archive="ripgrep-15.2.0-aarch64-unknown-linux-musl.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "armv7-unknown-linux-gnueabihf")
            ...
            _archive="ripgrep-15.2.0-armv7-unknown-linux-musleabihf.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "armv7-unknown-linux-musl-staticeabihf")
            ...
            ;;
        "armv7-unknown-linux-musleabi")
            _archive="ripgrep-15.2.0-armv7-unknown-linux-musleabi.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "x86_64-pc-windows-msvc")
            ...
            ;;
        "x86_64-unknown-linux-gnu")
            _archive="ripgrep-15.2.0-x86_64-unknown-linux-musl.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
...
install() {
    ...
    if [ -n "${RIPGREP_INSTALL_DIR:-}" ]; then
        _force_install_dir="$RIPGREP_INSTALL_DIR"
